            ...
            <div className=" w-full">
                ...
                <div className="flex flex-col gap-16 items-center justify-center w-full h-[calc(100vh-60px)]">
                    <div className="flex flex-col items-center gap-5">
                        <h1 className="text-4xl lg:text-6xl font-extrabold">Mon alternance</h1>
                    </div>
                    <div>
                        <Link href='#alternance-0'>
                            <button className="bg-btnColor px-4 py-2 rounded-md font-semibold hover:scale-105 transition-all">Découvrir</button>
                        </Link>
                    </div>
                </div>
                ...
                <div className="w-full flex flex-col items-center gap-24 mb-16">
                    {
                        alternances && alternances.length > 0 && alternances.map((item, index) => (
                            <div id={`alternance-${index}`} key={`alternance-${index}`} className='flex flex-col ps-5 pe-5 pb-5 md:pb-0 md:ps-2 md:pe-8 bg-white bg-opacity-10 rounded-lg  w-full md:flex-row md:max-w-[80%]'>
                                ...
                                <div className='flex flex-col justify-center gap-5 w-full md:w-9/12'>
                                    <p className='w-full text-base mt-4 font-bold'>{item.titre}</p>
                                    <p className='w-full text-sm mb-4 font-semibold'>{item.description}</p>
                                </div>
                            </div>
                        ))
                    }
